use ark_std::{fmt::Debug,string::String,marker::PhantomData,result::Result as IoResult, io::Error};
...
fn serialize_bytes_to_vec<O:Oracle>(r:Vec<u8>,veclength:usize, num_paths: Vec<usize>, path_lengths: Vec<usize>, leaf_indices: Vec<usize>) -> (Vec<<<O as Oracle>::C as Config>::InnerDigest>,Vec<Vec<Path<<O as Oracle>::C>>>){
	...
	let mut output = <<O as Oracle>::C as Config>::InnerDigest::deserialize(&mut val1);
	match output {
	...
	let ind_leaf_indices = &leaf_indices[start..(start + num_paths)];
	...
	for it in 0..num_paths{
	    ...
	    let leaf_index = ind_leaf_indices[it];
	    //for each pair in Path
	    let leaf_sibling_hash = <<O as Oracle>::C as Config>::LeafDigest::deserialize(&mut val1).ok().unwrap();
	    let mut pit = 0;
	    while pit <  path_length{
		let mut digest = <<O as Oracle>::C as Config>::InnerDigest::deserialize(&mut val1).ok();
	    ...
	    let path = Path::<<O as Oracle>::C>::set_path(leaf_sibling_hash, path_vec,leaf_index);
...
pub struct NiroaProof<O:Oracle>{
    pub roots: Vec<<<O as Oracle>::C as Config>::InnerDigest>,
...
pub type MerkleRoot<O:Oracle> = <<O as Oracle>::C as Config>::InnerDigest;
...
pub struct NiroaProver<O:Oracle,S:Setup,W:Witness>{
    ...
    pub oracle_hash : Box<dyn Fn(&O) -> (<<O as Oracle>::C as Config>::InnerDigest,MerkleTree<<O as Oracle>::C>)>
}

pub struct NiroaVerifier<S:Setup,O:Oracle>{
    //STUB - pass in challenges as temporary measure
    pub verifier_challenges : Vec<Box<dyn Fn(&S, &<<O as Oracle>::C as Config>::InnerDigest,&Vec<u8>) -> (<O as Oracle>::X,Vec<u8>)>>,
    ...
    pub verifier_query: Box<dyn Fn(&Vec<Vec<(<O as Oracle>::X,<O as Oracle>::Y)>>,&S, &Vec<<O as Oracle>::X>,&Vec<<<O as Oracle>::C as Config>::InnerDigest>,&Vec<Vec<Path<<O as Oracle>::C>>>) -> bool>//consider just passing in Proof here 
...
impl<O:'static + Oracle> CanonicalDeserialize for NiroaProof<O>{
    fn deserialize<R:Read>(mut reader:R)->IoResult<Self,SerializationError>{
	...
	let leaf_indices = Vec::<usize>::deserialize(&mut reader).ok().unwrap();
	let roots = Vec::<MerkleRoot<O>>::deserialize(&mut reader).ok().unwrap();
	let paths = Vec::<Vec<Path<<O as Oracle>::C>>>::deserialize(&mut reader).ok().unwrap();
//	let roots_and_paths:Vec<u8> = Vec::<u8>::deserialize(&mut reader).ok().unwrap();
	
	type F<O:Oracle> = <<O as Oracle>::C as Config>::InnerDigest; 

//	let (roots,paths) = serialize_bytes_to_vec::<O>(roots_and_paths,vec_length,num_paths,path_lengths,leaf_indices);
...
impl<O:'static + Oracle> CanonicalSerialize for NiroaProof<O>{
        fn serialize<W:Write>(&self,mut writer:W) -> Result<(),SerializationError>{
	    //first convert roots and paths to ToByte
...
//	    self.roots.write(&mut bytes_writer);
		...
		for path in p{
		    path_lengths.push(path.get_length());
    		    leaf_indices.push(path.leaf_index);
		}
	    }
	    
	    num_paths.serialize(&mut writer);
	    path_lengths.serialize(&mut writer);
	    leaf_indices.serialize(&mut writer);
	    self.roots.serialize(&mut writer);
	    ...
	    for p in &self.paths{
		for i in p{
		    i.leaf_sibling_hash.write(&mut bytes_writer);
		    for digest in &i.auth_path{
			digest.write(&mut bytes_writer);
		    }
...
pub fn iop_prover_to_niroa_prover<O:'static + Oracle,S:'static + Setup,W:'static + Witness>(iop_prover: Box<dyn Fn(&<O as Oracle>::X,&Vec<O>,&S,&W) -> O>) -> Box<dyn Fn(&<O as Oracle>::X,&Vec<O>,&S,&W) -> (O,<<O as Oracle>::C as Config>::InnerDigest,MerkleTree<<O as Oracle>::C>)> {
    let new_func = move |challenge: &<O as Oracle>::X, oracles: &Vec<O>, setup: &S,witness: &W| -> (O, <<O as Oracle>::C as Config>::InnerDigest,MerkleTree<<O as Oracle>::C>) {
...
pub fn iop_challenges_to_niroa_challenges<O:'static + Oracle,S:'static + Setup>() -> Box<dyn Fn(&S,&<<O as Oracle>::C as Config>::InnerDigest,&Vec<u8>) -> (<O as Oracle>::X,Vec<u8>)>{
    let new_func = move |setup:&S,root:&<<O as Oracle>::C as Config>::InnerDigest,hash_acc:&Vec<u8>| -> (<O as Oracle>::X,Vec<u8>) {
...
pub fn iop_get_queries_to_niroa_get_queries<O:'static + Oracle, S:'static + Setup>(
	...
	let new_func = move |oracles:&Vec<O>, trees: &Vec<MerkleTree<<O as Oracle>::C>>| -> (Vec<Vec<(<O as Oracle>::X,<O as Oracle>::Y)>>, Vec<Vec<Path<<O as Oracle>::C>>>) {
		...
		let mut round_paths  = Vec::new();
		for point in points {
		    let path = tree.generate_proof(oracles[i].position(&point.0)).unwrap();
		    round_paths.push(path)
		}
...
pub fn iop_query_to_niroa_query<O:'static + Oracle, S:'static + Setup>(
    ...
    Box<dyn Fn(&Vec<Vec<(<O as Oracle>::X,<O as Oracle>::Y)>>,&S, &Vec<<O as Oracle>::X>,&Vec<<<O as Oracle>::C as Config>::InnerDigest>,&Vec<Vec<Path<<O as Oracle>::C>>>) -> bool>{
	let new_func =
	    move
	    |query_points:&Vec<Vec<(<O as Oracle>::X,<O as Oracle>::Y)>>, setup: &S, challenges: &Vec<<O as Oracle>::X>, roots: &Vec<<<O as Oracle>::C as Config>::InnerDigest>, paths: &Vec<Vec<Path<<O as Oracle>::C>>> | -> bool{
...
fn oracle_hash_func<O:'static + Oracle>(oracle:&O) -> (<<O as Oracle>::C as Config>::InnerDigest,MerkleTree<<O as Oracle>::C>){
